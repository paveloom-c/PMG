...
\begin{figure}[H]
  ...
  \caption{Распределение объектов с несколькими локальными минимумами на профилях внутренней целевой функции (выборка HMSFRs, $ n = 3 $, 95 штук) в гелиоцентрической Галактической системе координат. Положительное направление оси X соответствует направлению на центр Галактики.}
  \label{fig:odd_objects}
\end{figure}
...
\hyperlink{toc}{\subsubsection{Результаты по полным выборкам}}
...
\begin{figure}[H]
  ...
  \caption{Кривая вращения для оптимальной модели, полученная по околосолнечной выборке. Дополнительные бары --- скорректированные неопределенности скоростей.}
\end{figure}
%
\begin{figure}[H]
  \centering
  ...
  \caption{Кривая вращения для оптимальной модели, полученная по выборке HMSFRs. Дополнительные бары --- скорректированные неопределенности скоростей.}
\end{figure}
%
\begin{figure}[H]
  \centering
...
\begin{figure}[H]
  \centering
  \subfloat{{\includegraphics[trim={0 0.5cm 0 0},clip,scale=1.9]{HMSFRs/n = 3/Parallaxes}}}\\
  \subfloat{{\includegraphics[scale=1.9]{HMSFRs/n = 3/Parallaxes (under 1 mas)}}}
  \caption{Соотношение исходных и приведенных параллаксов.}
  \label{fig:parallaxes}
\end{figure}

\newpage

\hyperlink{toc}{\subsubsection{Результаты с нулевыми ошибками параллаксов}}

Во многих работах (в том числе и в \cite{R.2019}) неопределенность параллаксов не учитывается. Для демонстрации эффекта игнорирования данной проблемы были получены результаты с отключенной внутренней оптимизацией.
...
\begin{figure}[H]
  ...
  \caption{Кривая вращения для оптимальной модели ($ n = 3 $), полученная по выборке HMSFRs с отключенной внутренней оптимизацией. Дополнительные бары --- скорректированные неопределенности скоростей.}
...
\begin{figure}[H]
  ...
  \caption{Кривая вращения для оптимальной модели ($ n = 4 $), полученная по выборке HMSFRs с отключенной внутренней оптимизацией. Дополнительные бары --- скорректированные неопределенности скоростей.}
  \label{fig:rotation_curves_hmsfrs_di_n_4}
\end{figure}

\newpage

\hyperlink{toc}{\subsubsection{Результаты по оптимальной выборке}}

По оптимальной выборке HMSFRs (после исключения выбросов) алгоритм был пропущен ещё раз, но уже с отключенными внутренней оптимизацией и проверками на выбросы.
...
По полным выборкам (\autoref{table:solar}, \autoref{table:hmsfrs}) оптимальные порядки для околосолнечной выборки и выборки HMSFRs определены как 1 и 3, соответственно. Результаты между выборками близки по сопоставимым параметрам, но для выборки HMSFRs они точнее. Профили целевой функции (\autoref{fig:profiles_hmsfrs}) для оптимальных порядков в основном гладкие, абсолютные ошибки параметров определяются точно, а относительные --- малы по величине. Ожидаемые детали на кривой вращения присутствуют (например, <<провал>> после солнечного круга). Зависимости $ L_1^{(1)}(n) $ и $ \sigma_\theta(n) $ подтверждают выбор оптимальных порядков.

Результаты для полного алгоритма были проверены на самосогласованность путем повторного пропуска с заменой начального значения $ R_0 $ на полученное при оптимизации. Значения параметров оказались практически идентичными итоговым.

Для вариантов с отключенной внутренней оптимизацией (\autoref{table:solar_di}, \autoref{table:hmsfrs_di}) оптимальный порядок однозначно не устанавливается (либо 3, либо 4). У $ n = 4 $ относительная ошибка старшего члена заметно меньше единицы и есть еще один точный член --- $ \theta_3 $. Кривая вращения для $ n = 4 $ выглядит на больших $ R $ более реалистично, чем для $ n = 3 $. Алгоритмы одномерного и трехмерного поиска выбросов исключили значительно больше объектов (для выборки HMSFRs это 42 объекта из 207-и вместо 14-ти). Это не удивительно: отклонения от модели теперь не объясняются ошибками в данных. В результате сокращения объема выборки почти для всех основных параметров (это параметры, отличные от $ \theta_i, \; i \ge 2 $), кроме $ R_0 $, точность ухудшилась. Для некоторых параметров есть заметные смещения: для $ A $, $ V_\odot $ и $ \theta_1 $. Радиальная и азимутальная дисперсия стали в 1.5--2 раза больше из-за того, что отклонения от модели, вызванные ошибками в расстояния, стали интерпретироваться как природная дисперсия. При этом эллипсоид скоростей стал нефизичным: азимутальная дисперсия превзошла радиальную. Вертикальная дисперсия не изменилась, т.к. для объектов, находящихся вблизи диска Галактики, ошибки в расстояниях почти не влияют на вертикальную скорость, а следовательно и на вертикальную дисперсию. Модельные кривые вращения допустимых порядков стали менее физичными: на больших $ R $ дают значительный формальный рост. Это потому, что у некоторых объектов при небольшой ошибке скорости имеется большая ошибка расстояния, а она не учитывается. Это аргумент в пользу необходимости учитывать неопределенность расстояний.
...
\begin{enumerate}
  ...
  \item Разработан, реализован и предоставлен в открытом доступе алгоритм пространственно"=кинематического моделирования однородной плоской подсистемы объектов Галактики, учитывающий случайные ошибки гелиоцентричиских расстояний, а также включающий ряд таких улучшений корректности, как: определение остаточного движение Солнца, оптимизация сглаженности закона вращения, гибкий алгоритм исключения выбросов в данных и другое. Метод применен к данным нового каталога.

  \item Расстояние от Солнца до центра Галактики $ R_0 $ по наиболее точной модели (выборка HMSFRs, n = 3) составляет $ 7.9205_{-0.1190}^{+0.1191} $ кпк. Получены оценки для всех фундаментальных параметров Галактики с малыми статистическими ошибками.

  \item Систематического смещения параллаксов не обнаружено: по оптимальной модели для выборки HMSFRs их среднее смещение $ \Delta\varpi $ составляет $ -0.95 \pm 1 $ мксд. Однако показано, что каталожные неопределенности параллаксов завышены на $ 1 / 5 $: $ \sigma = 0.804 \pm 0.041 $, а $ 1 / \sigma = 1.243 \pm 0.063 $
