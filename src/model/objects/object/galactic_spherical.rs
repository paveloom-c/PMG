    ...
    F: Float + FloatConst + Default + Debug,
{
        ...
        F2: Float + Debug + Into<F>,
    {
        ...
        // Compute the heliocentric velocity
        let (v_l, v_b) = self.compute_v_l_v_b_with(r_h.v, params);
        ...
        F2: Float + Debug + Into<F>,
    {
        ...
        // Compute the heliocentric velocity
        let (v_l, v_b) = self.compute_v_l_v_b_with(r_h.v, params);
        let (v_l_u, v_b_u) = self.compute_v_l_v_b_with(r_h.v_u, params);
        let (v_l_l, v_b_l) = self.compute_v_l_v_b_with(r_h.v_l, params);
        self.v_l = Some(Measurement {
            v: v_l,
            v_u: v_l_u,
            v_l: v_l_l,
